...
function fm_pa_defaults(string $block): array {
    static $cache = [];
    if (!in_array($block, ['pulse-atelier', 'pulse-atelier-section'], true)) { return []; }
    return $cache[$block] ??= json_decode(file_get_contents(__DIR__ . '/../' . $block . '/defaults.json'), true);
}
...
function fm_pa_image(array $images, string $name): string {
    ...
    return FM_BLOCKS_URL . 'src/templates/pulse-atelier/assets/' . $name . '.jpg';
}
