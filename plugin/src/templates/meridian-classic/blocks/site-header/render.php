...
<div class="meridian meridian-header" data-meridian-header>
  <div class="meridian-header__bar">
    ...
    <nav class="meridian-nav" id="<?php echo esc_attr($uid); ?>" aria-label="Main navigation">
      <button type="button" class="meridian-menu-close" data-menu-close aria-label="Close navigation"><svg aria-hidden="true" viewBox="0 0 24 24"><path d="M5 5l14 14M19 5L5 19"/></svg></button>
      <ul>
        <?php foreach ($nav as $item) : ?>
          <li><a href="<?php echo esc_url($item['url']); ?>" <?php echo !empty($item['current']) ? 'aria-current="page"' : ''; ?>><?php echo esc_html($item['label']); ?></a></li>
        <?php endforeach; ?>
        <?php if ($setting('cta_url') && $setting('cta_label')) : ?><li class="meridian-nav__desktop-cta"><a href="<?php echo esc_url($setting('cta_url')); ?>"><?php echo esc_html($setting('cta_label')); ?></a></li><?php endif; ?>
    ...
    <button type="button" class="meridian-menu-toggle" aria-expanded="false" aria-controls="<?php echo esc_attr($uid); ?>" aria-label="Open navigation"><span aria-hidden="true"></span><span aria-hidden="true"></span><span aria-hidden="true"></span></button>
  </div>
</div>
