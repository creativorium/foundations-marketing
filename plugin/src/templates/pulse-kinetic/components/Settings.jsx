...
export default function Settings({ defaults, value = {}, onChange }) {
  ...
  return <>
    ...
    {Object.entries(defaults.lists).map(([key, rows]) => <PanelBody key={key} title={label(key)} initialOpen={false}>
      <Repeater
        items={value.lists?.[key] ?? rows}
        blank={rows[0] ?? {}}
        label={(item, index) => item.name || item.title || item.day || item.author || item.q || `Item ${index + 1}`}
        onChange={(next) => change('lists', { ...value.lists, [key]: next })}
      >
    ...
    {Object.keys(defaults.images).length > 0 && <PanelBody title="Photography" initialOpen={false}>
      {Object.keys(defaults.images).map((key) => <div key={key}>
        <TextControl label={`${label(key)} image URL`} value={value.images?.[key] ?? ''} onChange={(url) => change('images', { ...value.images, [key]: url })} />
